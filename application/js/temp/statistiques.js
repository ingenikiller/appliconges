$(document).ready(function() {
	afficheFluxSelect('fluxdispos', $('#numeroCompte').val());

	$("#fluxdispos option").dblclick(function(){
		if($(this).val()!='')
        	$('#fluxselectionnes').append($(this));
    });

    /*$("#fluxselectionnes option").dblclick(function(){
		//if($(this).val()!=''){
			alert("fdf");
			$(this).remove();
		//}
        	//$('#fluxdispos').append($(this));
    });*/
});
...
/*
	formulaire sélection flux
*/
function soumettreFlux(form) {
	if(!validForm(form)) {
		return false;
	}
	
	var listeflux="";
	var tableauOptions=$('#fluxselectionnes')[0].options;
	for (i=0; i<tableauOptions.length; i++){
		listeflux+="|"+tableauOptions[i].value +"";
	}


	$.ajax({ 
	    url: "index.php?domaine=statistique&service=statflux",
	    data: { "numeroCompte": form.numeroCompte.value,
	    		"premiereAnnee": form.premiereAnnee.value,
				"derniereAnnee": form.derniereAnnee.value,
				"listeflux": listeflux
